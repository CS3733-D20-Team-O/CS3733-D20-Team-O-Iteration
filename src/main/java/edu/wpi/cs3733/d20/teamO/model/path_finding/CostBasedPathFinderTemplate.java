...
abstract class CostBasedPathFinderTemplate implements PathFinder {

  protected abstract double calculateWeight(double currentDistance, double distanceToGo);
    ...
    while (!frontier.isEmpty()) {
      ...
      for (val n : current.getNeighbors()) {
        ...
        if (!costSoFar.containsKey(n) || newCost < costSoFar.get(n)) {
          ...
          frontier.add(new NodeWithPriority(n, calculateWeight(newCost, distanceBetween(n, stop))));
   ...
   */
  @Value
  protected static class NodeWithPriority implements Comparable<NodeWithPriority> {
